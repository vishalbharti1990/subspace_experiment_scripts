...
if(length(args) != 4) {
  cat("USAGE: Rscript run_ORCLUS.R <data_dir_root> <data_name> <result_root_dir> <reps>\n")
...
result_file <-  file.path(args[3], args[2], 'ORCLUS/results.csv')

# Create result dir if not already existing
dir.create(file.path(args[3], args[2], 'ORCLUS'), showWarnings = F, recursive = T)
...
cat("RUNNING ORCLUS FOR ", args[2], " DATASET..\n")

num_points <- length(list.files(p, pattern="*\\.p$", full.names=TRUE))

# loop through the datasets and run the SUBCLU algorithm
for (feats in data_feats) {
  ...
  for (k in 1:reps) {
    ...
    res <- orclus(data, k=10, l=50, k0=20, a=0.25)
