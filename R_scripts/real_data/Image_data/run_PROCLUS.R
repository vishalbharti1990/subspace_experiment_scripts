...
if(length(args) != 4) {
  cat("USAGE: Rscript run_PROCLUS.R <data_dir_root> <data_name> <result_root_dir> <reps>\n")
...
result_file <-  file.path(args[3], args[2], 'PROCLUS/results.csv')

# Create result dir if not already existing
dir.create(file.path(args[3], args[2], 'PROCLUS'), showWarnings = F, recursive = T)
...
cat("RUNNING PROCLUS FOR ", args[2], " DATASET..\n")
