...
if(length(args) != 3) {
  cat("USAGE: Rscript run_CLIQUE.R <data_dir> <result_root_dir> <reps>\n")
...
res_anuran_data_file <- file.path(args[2], 'CLIQUE/results.csv')

# Create result dir if not already existing
dir.create(file.path(args[2], 'CLIQUE'), showWarnings = F, recursive = T)
...
for (k in 1:reps) {
  ...
  cat("Reps =", k, nmi_species, nmi_genus, nmi_families, "\n")
}
