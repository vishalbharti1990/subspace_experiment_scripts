## SCRIPT TO RUN PROCLUS CLUSTERING ON ANURAN CALLS DATASET
library(subspace)
library(tictoc)
...
if(length(args) != 3) {
  cat("USAGE: Rscript run_PROCLUS.R <data_dir> <result_root_dir> <reps>\n")
...
res_anuran_data_file <- file.path(args[2], 'PROCLUS/results.csv')

# Create result dir if not already existing
dir.create(file.path(args[2], 'PROCLUS'), showWarnings = F, recursive = T)
