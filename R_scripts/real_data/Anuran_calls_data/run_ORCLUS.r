## SCRIPT TO RUN ORCLUS CLUSTERING ON ANURAN CALLS DATASET
library(orclus)
library(tictoc)
...
if(length(args) != 3) {
  cat("USAGE: Rscript run_ORCLUS.R <data_dir> <result_root_dir> <reps>\n")
...
res_anuran_data_file <- file.path(args[2], 'ORCLUS/results.csv')

# Create result dir if not already existing
dir.create(file.path(args[2], 'ORCLUS'), showWarnings = F, recursive = T)
...
# run proclus for species (k=10)
for (k in 1:reps) {
  
  # run the ORCLUS algorithm
  res <- orclus(X, k = 10, l = 5, 20, 0.5, verbose = FALSE)
...
# run proclus for genus (k=8)
for (k in 1:reps) {
  
  # run the ORCLUS algorithm
  res <- orclus(X, k = 8, l = 5, 16, 0.5, verbose = FALSE)
...
# run proclus for families (k=4)
for (k in 1:reps) {
  
  # run the ORCLUS algorithm
  res <- orclus(X, k = 4, l = 5, 8, 0.5, verbose = FALSE)
