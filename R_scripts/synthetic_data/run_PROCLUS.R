## SCRIPT TO RUN PROCLUS CLUSTERING ON SYNTHETIC DATASET
library(subspace)
library(tictoc)
...
if(length(args) != 3) {
  cat("USAGE: Rscript run_PROCLUS.R <data_root_dir> <result_root_dir> <reps>\n")
...
result_axis_par_file   <- file.path(args[2], 'axis_parallel/PROCLUS/results.csv')

result_arbitrary_file  <- file.path(args[2], 'arbitrary/PROCLUS/results.csv')

# Create result dir if not already existing
dir.create(file.path(args[2], 'axis_parallel/PROCLUS'), showWarnings = F, recursive = T)
dir.create(file.path(args[2], 'arbitrary/PROCLUS'), showWarnings = F, recursive = T)
...
cat("RUNNING PROCLUS FOR AXIS PARALLEL DATASET..\n")
...
for (data_file in data_axis_parallel) {
  ...
  for (k in 1:reps) {
    ...
    # run the PROCLUS algorithm
    res <- ProClus(data, k = num_clus, d = avg_dim)
...
cat("\nRUNNING PROCLUS FOR ARBITRARY ORIENTED DATASET..\n")
