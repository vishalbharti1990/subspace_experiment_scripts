## SCRIPT TO RUN ORCLUS CLUSTERING ON SYNTHETIC DATASET
library(orclus)
library(tictoc)
...
if(length(args) != 3) {
  cat("USAGE: Rscript run_ORCLUS.R <data_root_dir> <result_root_dir> <reps>\n")
...
result_axis_par_file   <- file.path(args[2], 'axis_parallel/ORCLUS/results.csv')

result_arbitrary_file  <- file.path(args[2], 'arbitrary/ORCLUS/results.csv')

# Create result dir if not already existing
dir.create(file.path(args[2], 'axis_parallel/ORCLUS'), showWarnings = F, recursive = T)
dir.create(file.path(args[2], 'arbitrary/ORCLUS'), showWarnings = F, recursive = T)
...
cat("RUNNING ORCLUS FOR AXIS PARALLEL DATASET..\n")

# loop through the datasets and run the ORCLUS algorithm
for (data_file in data_axis_parallel) {
  ...
  # compute average subspace dimension
  avg_dim <- round(min(5, dim(data)[2]-1)/2)
  
  # extract number of clusters
  num_clus <- as.numeric(strsplit(f_name[length(f_name)],'_')[[1]][3])
  ...
  for (k in 1:reps) {
    ...
    # run the orclus algorithm
    res <- orclus(data, num_clus, avg_dim, num_clus*2, 0.5, verbose = FALSE)
...
cat("\nRUNNING ORCLUS FOR ARBITRARY ORIENTED DATASET..\n")
