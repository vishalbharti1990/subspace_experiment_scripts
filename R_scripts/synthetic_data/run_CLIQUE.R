...
## CLIQUE doesn't require a random seed as it is a deterministic algorithm
library(subspace)
library(tictoc)
...
if(length(args) != 3) {
  cat("USAGE: Rscript run_CLIQUE.R <data_root_dir> <result_root_dir> <reps>\n")
...
result_axis_par_file   <- file.path(args[2], 'axis_parallel/CLIQUE/results.csv')

result_arbitrary_file  <- file.path(args[2], 'arbitrary/CLIQUE/results.csv')

# Create result dir if not already existing
dir.create(file.path(args[2], 'axis_parallel/CLIQUE'), showWarnings = F, recursive = T)
dir.create(file.path(args[2], 'arbitrary/CLIQUE'), showWarnings = F, recursive = T)
...
cat("RUNNING CLIQUE FOR AXIS PARALLEL DATASET..\n")
...
cat("\nRUNNING FOR ARBITRARY ORIENTED DATASET..\n")
...
for (data_file in data_arbitrary) {
  ...
  # arrays to store the results for current dataset
  NMI_sqrt = array(data = 0, dim = 100)
  NMI_min  = array(data = 0, dim = 100)
  NMI_max  = array(data = 0, dim = 100)
  NMI_sum  = array(data = 0, dim = 100)
